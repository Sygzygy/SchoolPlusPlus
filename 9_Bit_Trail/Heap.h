...
public:
    Branch(unique_ptr<Node> leftNode, unique_ptr<Node> rightNode)
        : left(move(leftNode)), right(move(rightNode)),
        combinedSymbol(left->characterSymbol() + right->characterSymbol()),
        combinedFrequency(left->characterFrequency() + right->characterFrequency()),
        nodePriority(calculateAsciiPriority(this->combinedSymbol)) {}

    float characterFrequency() override { return combinedFrequency; }
    string characterSymbol() override { return combinedSymbol; }
    int priority() override { return nodePriority; }

    Node* getLeft() const { return left.get(); } // Raw pointer for access
    Node* getRight() const { return right.get(); } // Raw pointer for access
