...
public:
    ...
    CharType& getASCIIValue() {  // Non-const overload for mutable access
        return std::get<2>(*this);
    }
    const CharType& getASCIIValue() const {  // Const overload for read-only access
        return std::get<2>(*this);
    }
...
public:
    ...
    auto findByKey(const BitsetType& key) {
    ...
    auto findByString(const StringType& str) {
        return find_if(this->begin(), this->end(),
            [&str](const TupleType& manager) {
                return manager.getString() == str;
            });
